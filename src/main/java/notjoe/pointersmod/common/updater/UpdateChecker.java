...
public class UpdateChecker {
    ...
    public UpdateStatus checkForUpdates() {
        ...
        UpdateStatus status = new UpdateStatus(releaseType, jsonData.get("alpha").toString(),
            jsonData.get("beta").toString(), jsonData.get("release").toString());

        try {
            reader.close();
            is.close();
        } catch (IOException e) {
            PointersMod.logger.warn(
                "An error occurred while finishing update checking. Its stack trace follows.");
            e.printStackTrace();
        }
