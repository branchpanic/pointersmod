...
public class PointerActionInventory extends PointerAction {
    public static IItemHandler getStackHandlerFromInvPointer(ItemStack stack, World world,
        EntityPlayer player) {
        if (stack.getItem() == ModItems.pointer_inv) {
            // jesus christ
            return ((PointerActionInventory) (((ItemPointerBase) ModItems.pointer_inv).pointerAction))
                .getStackHandler(stack, world, player);
        }
    ...
    public boolean setPointerTarget(ItemStack stack, BlockDetail blockDetail, World world) {
        ...
        if (blockDetail.isTileEntity(world)) {
            if (blockDetail.getTileEntity(world)
                .hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, blockDetail.facing)) {
    ...
    @Override public boolean pointerActivated(ItemStack stack, World world, EntityPlayer player) {
        if (hasTarget(stack) && isTargetAccessible(stack, world, player) && !world.isRemote) {
            ...
            IItemHandler handler = blockDetail.getTileEntity(world)
                .getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, blockDetail.facing);
            if (handler != null) {
            ...
            for (int i = 0; i < player.inventory.mainInventory.length; i++) {
                if (player.inventory.mainInventory[i] != null
                    ...
                    player.inventory.mainInventory[i] =
                        moveStackToHandler(player.inventory.mainInventory[i], targetHandler);
                }
            }
    ...
    @Override public boolean isTargetAccessible(ItemStack stack, World world, EntityPlayer player) {
        ...
        return super.isTargetAccessible(stack, world, player) &&
            target.isTileEntity(world) && target.getTileEntity(world)
            .hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, target.facing);
